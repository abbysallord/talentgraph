...
export const AdminCockpit: React.FC = () => {
  ...
  if (loading && !stats) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-zinc-200 shadow-xs">
        <RefreshCw className="w-8 h-8 text-zinc-900 animate-spin mb-3" />
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      {/* Header Banner */}
      <div className="bg-white rounded-2xl border border-zinc-200 p-6 md:p-8 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-zinc-100 border border-zinc-200 text-zinc-800 text-xs font-bold uppercase tracking-wider mb-2">
            <Activity className="w-3.5 h-3.5" />
            <span>Telemetry & Hiring Cockpit</span>
          </div>
          <h2 className="text-2xl md:text-3xl font-extrabold text-zinc-900 font-display">
        ...
        <div className="flex items-center space-x-3">
          <div className="flex items-center space-x-2 px-3 py-1.5 rounded-xl bg-zinc-50 border border-zinc-200 text-xs text-zinc-600 font-mono">
          ...
          <button
            onClick={fetchStats}
            disabled={loading}
            className="inline-flex items-center space-x-2 px-4 py-2 rounded-xl bg-white border border-zinc-200 text-zinc-700 text-xs font-bold hover:bg-zinc-50 transition-colors shadow-2xs"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 p-6 shadow-xs">
          ...
          <div className="space-y-4">
            {[
              ...
              { stage: "Stage 3: Multi-Dimensional Reasoning", key: "REASONING", count: stats?.stageCounts?.REASONING || 1, color: "bg-zinc-1000" },
              ...
              return (
                <div key={item.key} className="space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-semibold text-zinc-700">{item.stage}</span>
          ...
          <div className="mt-8 pt-6 border-t border-zinc-100">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
              <div className="p-2.5 rounded-xl bg-zinc-100 border border-zinc-200 text-center">
                <span className="block text-xs font-bold text-zinc-900">Strong Hire</span>
                <span className="text-lg font-extrabold text-zinc-950 font-display">
              ...
              <div className="p-2.5 rounded-xl bg-zinc-50 border border-zinc-200 text-center">
                <span className="block text-xs font-bold text-zinc-700">Hold</span>
                <span className="text-lg font-extrabold text-zinc-900 font-display">
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 p-6 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-zinc-50 border border-zinc-200 flex items-start space-x-3">
                <CheckCircle2 className="w-5 h-5 text-zinc-900 shrink-0 mt-0.5" />
                <div>
                  <h4 className="text-sm font-bold text-zinc-900">Deterministic Evidence Extraction</h4>
              ...
              <div className="p-4 rounded-xl bg-zinc-50 border border-zinc-200 flex items-start space-x-3">
                <CheckCircle2 className="w-5 h-5 text-zinc-900 shrink-0 mt-0.5" />
                <div>
                  <h4 className="text-sm font-bold text-zinc-900">Zero-Hallucination Written Rationale</h4>
                  <p className="text-xs text-zinc-600 mt-0.5">
              ...
              <div className="p-4 rounded-xl bg-zinc-50 border border-zinc-200 flex items-start space-x-3">
                <CheckCircle2 className="w-5 h-5 text-zinc-900 shrink-0 mt-0.5" />
                <div>
                  <h4 className="text-sm font-bold text-zinc-900">Human Recruiter Veto Authority</h4>
                  <p className="text-xs text-zinc-600 mt-0.5">
          ...
          <div className="flex items-center space-x-2">
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1.5 rounded-xl border border-zinc-200 bg-zinc-50 text-xs text-zinc-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-zinc-1000"
              />
            </div>
            <select
              value={filterType}
              onChange={(e) => setFilterType(e.target.value)}
              className="px-3 py-1.5 rounded-xl border border-zinc-200 bg-zinc-50 text-xs font-semibold text-zinc-700 focus:bg-white focus:outline-none focus:ring-2 focus:ring-zinc-1000"
            >
            ...
            <tbody className="divide-y divide-zinc-100">
              {filteredLogs.length === 0 ? (
                ...
                filteredLogs.map((log: any) => {
                  ...
                  if (log.eventType === "EVALUATION_RUN") badgeColor = "bg-zinc-100 text-zinc-800 border border-zinc-300";
                  ...
                  if (log.eventType === "LOGIN") badgeColor = "bg-zinc-100 text-zinc-800 border border-zinc-300";
                      ...
                      <td className="py-3 px-4 font-semibold text-zinc-900">
                        {log.candidateName ? (
                          <div>
                            ...
                            <div className="text-2xs text-zinc-500 font-normal">{log.candidateRole}</div>
                          </div>
                        ) : (
                          <span className="text-zinc-400">System Level</span>
                        )}
                      </td>
                      <td className="py-3 px-4 text-zinc-700">
                        <div className="font-semibold">{log.userName}</div>
                        <div className="text-2xs text-zinc-400">{log.userEmail}</div>
                      </td>
                      <td className="py-3 px-4 text-zinc-600 max-w-xs truncate font-mono text-2xs">
                        {JSON.stringify(log.details)}
                      </td>
                      <td className="py-3 px-4 text-zinc-500 font-mono text-2xs whitespace-nowrap">
